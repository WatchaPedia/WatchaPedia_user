...
try{
    ...
    for(let idx of container) {
        ...
        let scrollMax = scrollBox.scrollWidth - idx.querySelector('.css-119xxd7').getBoundingClientRect().width;
        // 스크롤 위치에 따라 버튼 유무
    ...
    for(let idx of container){
        idx.querySelector('.css-vp7uyl').addEventListener('click', function () {
            idx.querySelector('.css-awu20a').scrollBy(idx.querySelector('.css-awu20a').getBoundingClientRect().width+7,0)
        })
