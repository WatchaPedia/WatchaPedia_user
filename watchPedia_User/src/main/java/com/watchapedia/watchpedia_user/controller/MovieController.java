...
//        해당 유저가 별점을 매겼는지
        StarResponse hasStar = starService.findStar("movie",movie.idx(), user.getUserIdx());
...
//      해당 유저가 코멘트를 달았는지
        ...
        boolean hasWish = wishService.findWish("movie",movie.idx(),user.getUserIdx());
        boolean hasWatch = watchService.findWatch("movie",movie.idx(),user.getUserIdx());
        boolean hasHate = hateService.findHate(user,"movie",movie.idx());
